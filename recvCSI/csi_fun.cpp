...
void fill_csi_matrix(u_int8_t *csi_addr, int nr, int nc, int num_tones, COMPLEX (*csi_matrix)[3][114], char *filename)
{
    ofstream file[nc][nr];
    string s[nc][nr];
    for (int i = 0; i < nc; i++)
    {
        for (int j = 0; j < nr; j++)
        {
            s[i][j] = string(filename) + "/csv";
            s[i][j] += std::to_string(i + 1) + "x" + std::to_string(j + 1) + ".csv";
            file[i][j].open(s[i][j].c_str(), std::ios_base::app);
        }
    }
    /*s = filename;
    s += "1.csv";
    file1.open(s.c_str(), std::ios_base::app);
    s = filename;
    s += "2.csv";
    file2.open(s.c_str(), std::ios_base::app);*/
    ...
    for (k = 0; k < num_tones; k++)
    {
        ...
        for (nc_idx = 0; nc_idx < nc; nc_idx++)
        {
            ...
            for (nr_idx = 0; nr_idx < nr; nr_idx++)
            {
                ...
                //cout << "NR:" << unsigned(nr_idx) << ",  real:" << real << ",  imag:" << imag << endl;
                /*if (nc_idx == 0 && nr_idx == 1)
                {
                    file1 << sqrt(real * real + imag * imag) << ",";
                    cout << "nc:" << unsigned(nc_idx) << ", real:" << setw(5) << real << ", imag:" << setw(5) << imag << ", val:" << setw(5) << sqrt(real * real + imag * imag) << endl;
                }
                if (nc_idx == 1 && nr_idx == 1)
                {
                    file2 << sqrt(real * real + imag * imag) << ",";
                    cout << "nc:" << unsigned(nc_idx) << ", real:" << setw(5) << real << ", imag:" << setw(5) << imag << ", val:" << setw(5) << sqrt(real * real + imag * imag) << endl;
                }*/
                //cout << "at:" << unsigned(nc_idx) << "x" << unsigned(nr_idx) << ", real:" << setw(5) << real << ", imag:" << setw(5) << imag << ", val:" << setw(5) << sqrt(real * real + imag * imag) << endl;
                file[nc_idx][nr_idx] << sqrt(real * real + imag * imag) << ",";
...
void record_csi_payload(unsigned char *buf_addr, csi_struct *csi_status, unsigned char *data_buf, COMPLEX (*csi_matrix)[3][114], char *filename)
{
    int i;
    int nr, nc, num_tones;
    u_int8_t *csi_addr;
    ...
    fill_csi_matrix(csi_addr, nr, nc, num_tones, csi_matrix, filename);
}
